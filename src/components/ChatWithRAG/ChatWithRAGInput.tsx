...
import { CSNode } from '../../services/folders'

interface ChatInputProps {
  onSend: (message: string, attachments?: Attachment[], selectedFileNodeIds?: number[]) => void
  disabled?: boolean
  isLoading?: boolean
  isUploading?: boolean
  onStop?: () => void
...
export const ChatWithRAGInput = forwardRef<ChatInputHandle, ChatInputProps>(function ChatInput(
  { onSend, disabled, isLoading, isUploading, onStop, attachments = [], onRemoveAttachment, onAddFiles },
  ref
) {
  const [input, setInput] = useState('')
  const [selectedFiles, setSelectedFiles] = useState<CSNode[]>([])
      ...
      {selectedFiles.length > 0 && (
        <div className="chat-selected-files">
          {selectedFiles.map(file => (
            <div key={file.dataID} className="chat-selected-file">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
                <polyline points="14 2 14 8 20 8" />
              </svg>
              <span className="chat-selected-file-name">{file.name}</span>
              <button
                type="button"
                onClick={() => setSelectedFiles(selectedFiles.filter(f => f.dataID !== file.dataID))}
